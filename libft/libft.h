...
/*
	Returns a node string of size `len` that starts from
...
/*
	Returns a node string which is the result of applying
...
/*
	...
	followed by a nodeline character.
 ...
 * (Previous content gets pushed after the node head).
...
/*
	Returns a node list with `f` applied on `lst` and
